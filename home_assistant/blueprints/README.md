...
`README.md` del repo raíz, sección "Calibración de persianas". Desde
la **1.8.0**, esa posición también se puede **comandar**:
`cover.set_cover_position` mueve la persiana de verdad, igual que
`cover.open_cover` / `cover.close_cover` / `cover.stop_cover`,
directamente sobre `cover.persiana_XX_YY` — sin helpers ni blueprints
intermedios.

⚠️ **Requiere el parche de `ArduinoHA` en
[`mega_dispositivos/lib_overrides/`](../../mega_dispositivos/lib_overrides/README.md)
instalado, además del firmware 1.8.0+.** La librería `ArduinoHA` de
fábrica (Library Manager, sin parchear) solo permite *reportar*
posición, nunca *recibir* comandos de posición desde HA — sin el
parche, `cover.set_cover_position` no hace nada sobre estas entidades,
sin error visible. No es un problema de configuración; es una
limitación real de la librería, documentada con detalle en el README
del parche.
...
anteriores a 1.6.0 sin reporte de posición real) y
`persiana_ir_a_posicion.yaml` (el script-workaround usado mientras
`set_cover_position` no funcionaba, entre la detección del problema y
el parche de la 1.8.0) quedan ambos en
[`legacy/`](legacy/README.md) — hoy ninguno hace falta, usa
`cover.set_cover_position` directamente.
...
1, 2 y 5 pulsaciones, y `cover.set_cover_position` para 3 y 4 (ir a un
% concreto). Requiere `mega_dispositivos` 1.8.0+ con el parche de
`lib_overrides/` instalado (ver aviso más arriba) — en firmware más
antiguo o sin el parche, `set_cover_position` no hace nada y las
pulsaciones 3/4 no tendrán efecto.
...
2. **Pulsador (device)**: el device MQTT donde están cableados ambos
   botones.
3. **Subtype del botón de SUBIR** y **Subtype del botón de BAJAR**:
   los dos pines de la pareja, p. ej. `p22` y `p23`.
...
> ⚠️ **Cambio incompatible en `v2.0.0`.** Los inputs han cambiado
> (`boton_subtype` → `boton_subtype_subir` + `boton_subtype_bajar`, y
> desaparece `direccion`), así que las automatizaciones creadas con
> `v1.x` **dejan de funcionar** al reimportar el blueprint. Hay que
> recrearlas: una nueva por persiana, en vez de las dos que había.
...
mismo servicio que usa `persiana_pulsador_completo.yaml` (pulsaciones
3/4) y cualquier slider manual, sin ningún helper ni script
intermedio. Requiere el parche de `lib_overrides/` instalado (ver
aviso al principio de este README).
...
tarjeta, el pulsador físico, o cualquier llamada a
`set_cover_position`), la integración lo detecta y dejaría de forzar
su cálculo hasta que se le devuelva el control (normalmente vía un
`switch`/botón que expone la propia integración, del tipo "reanudar
control automático" — el nombre exacto depende de la versión
instalada; revisarlo al dar de alta la instancia real).
...
devolver el control). Ejemplo de cómo quedaría la automatización con
esa condición añadida:
