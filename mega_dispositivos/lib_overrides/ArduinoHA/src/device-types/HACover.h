...
#define HACOVER_POSITION_CALLBACK(name) void (*name)(const int16_t position, HACover* sender)
 ...
 *
 * PATCHED (proyecto ruben_smart_home_arduino): esta versión añade soporte
 * para recibir comandos de posición (`set_position_topic`), que la
 * librería original no implementa — `PositionFeature` en la versión
 * original solo permite REPORTAR posición, nunca comandarla. Ver
 * `mega_dispositivos/lib_overrides/README.md` para el detalle de qué se
 * ha tocado y por qué.
 */
...
public:
    ...
    /**
     * PATCHED: Registers callback that will be called each time a "go to position X"
     * command is received from HA (`cover.set_cover_position`). Only works if
     * `PositionFeature` is enabled — without it, HA never shows the position
     * slider/service in the first place. Position is clamped to 0-100 before
     * the callback runs.
     *
     * @param callback
     */
    inline void onPositionCommand(HACOVER_POSITION_CALLBACK(callback))
        { _positionCommandCallback = callback; }
...
private:
    ...
    /**
     * PATCHED: Parses a numeric "set position" payload and executes the
     * position callback, clamped to 0-100.
     *
     * @param cmd The data of the command.
     * @param length Length of the command.
     */
    void handlePositionCommand(const uint8_t* cmd, const uint16_t length);
    ...
    /// PATCHED: The callback invoked when HA sends a "go to position X" command.
    HACOVER_POSITION_CALLBACK(_positionCommandCallback);
};
