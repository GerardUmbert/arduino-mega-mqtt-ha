...
1, 2 y 5 pulsaciones, y `cover.set_cover_position` para 3 y 4 (ir a
50%, ±5%).

!!! warning "Requiere mega_dispositivos 1.8.0+ con el parche de ArduinoHA"
    `cover.set_cover_position` solo funciona sobre estas entidades con
    `mega_dispositivos` 1.8.0+ **y** el parche de
    `mega_dispositivos/lib_overrides/` instalado (ver
    [índice de blueprints](index.md)) — la librería `ArduinoHA` de
    fábrica no acepta comandos de posición. Sin eso, las pulsaciones
    3/4 no tendrán efecto, sin error visible.
